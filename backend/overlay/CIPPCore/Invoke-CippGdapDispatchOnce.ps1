function Invoke-CippGdapDispatchOnce {
    ...
    if ($Id -cnotmatch '^[A-Za-z0-9][A-Za-z0-9_-]{0,255}$') { throw 'Invalid relationship identifier.' }
    $SafeId = ConvertTo-CIPPODataFilterValue -Value $Id -Type String
    $Onboarding = Get-CIPPTable -TableName TenantOnboarding
    if (Get-CIPPAzDataTableEntity @Onboarding -Filter "PartitionKey eq 'Onboarding' and RowKey eq '$SafeId'" -First 1) { return }
    ...
    try {
        ...
        if (Get-CIPPAzDataTableEntity @Claims -Filter "PartitionKey eq 'Dispatch' and RowKey eq '$SafeId'" -First 1) { return }
        throw
    }
